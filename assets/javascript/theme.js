const themes = () => {
  if (secret === 0) {
    terminal.insertAdjacentHTML(
      ...
      `<p class="result">
        <ul>
          <li>Here is the list of <code>themes</code> available :</li>
          <ol>
            <li>dracula</li>
            <li>dark</li>
            <li>ubuntu</li>
        ...
        <p class="result">You can choose a theme by writing in the order the name of your theme or the number of the latter. Then confirm your order by pressing <em>Enter</em></p><br>
...
const secrets = () => {
  if (secret === 0) {
    ...
    terminal.insertAdjacentHTML(
      'beforeend',
      `<p class="result">
          Secret themes are already active. Use the command <code>themes</code> to see your themes. 
        </p>`
    );
  }
};
