  ...
  if (e.key === 'Enter') {
    if (result === '') {
      ...
      answer.insertAdjacentHTML(
        'beforeend',
        `<p class="resultwrite">${input.value}</p>`
      );
      i++;
        ...
        if (
          ...
          if (main.classList.contains('glassmorphism')) {
            terminal.insertAdjacentHTML(
              'beforeend',
              `${data[language].glassmorphismOff}`
            );
          } else {
            terminal.insertAdjacentHTML(
              'beforeend',
              `${data[language].glassmorphismOn}`
            );
          }
        } else {
          ...
          terminal.insertAdjacentHTML(
            'beforeend',
            `<p class="notfound result">${result}: command not found</p>`
          );
        }
      } else if (y === 0) {
        if (result === 'help') {
      ...
      if (y === -1) {
        ...
        terminal.insertAdjacentHTML(
          'beforeend',
          `<p class="commande" id="answer${i}">ptrckyrd_x:~$:</p><input type="text" autofocus class="input themeinput" id="input${i}" tabindex="-1">`
        );
      } else if (y === 1) {
        terminal = document.getElementById('terminal');
        terminal.insertAdjacentHTML(
          'beforeend',
          `<p class="commande" id="answer${i}">ptrckyrd_x:~/portfolio/themes$:</p><input type="text" autofocus class="input themeinput" id="input${i}" tabindex="-1">`
        );
      } else {
        terminal = document.getElementById('terminal');
        terminal.insertAdjacentHTML(
          'beforeend',
          `<p class="commande" id="answer${i}">ptrckyrd_x:~/portfolio$</p><input type="text" autofocus class="input" id="input${i}" tabindex="-1">`
        );
      }
      document.getElementById(`input${i}`).focus();
      if (
        document.getElementById(`input${i}`).classList.contains('themeinput') &&
        y === -1
      ) {
