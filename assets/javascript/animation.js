...
const moveBg = (historic, result, y) => {
  if (result === 'move' && document.body.classList.contains('animateBg')) {
    ...
    terminal.insertAdjacentHTML(
      'beforeend',
      `<p class="result">the background animation is already active. To disable the latter use the command <code>remove</code>.</p>`
    );
  } else if (result === 'move') {
    ...
    terminal.insertAdjacentHTML(
      'beforeend',
      `<p class="result">You have just activated the animation of the background to deactivate it use the command <code>remove</code>.</p>`
    );
  } else if (
    result === 'remove' &&
    ...
    terminal.insertAdjacentHTML(
      'beforeend',
      `<p class="result">You have just deactivated the animation of the background to activate the latter use the command <code>move</code>.</p>`
    );
  } else {
    if (y === 1) {
      terminal = document.getElementById('terminal');
